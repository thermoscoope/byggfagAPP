...
# [cite_start]Mørkt tema og stil (Beholdt fra din kode) [cite: 1, 2]
st.markdown("""
    <style>
...
# [cite_start]2. Initialisering [cite: 3]
...
if not st.session_state.startet:
    ...
    st.markdown("""
    ### Klar for å bli en mester i byggfag?
    I denne appen skal vi utforske de ulike retningene innen bygg og anlegg. 
    Du vil lære om verktøy, utdanning, og ikke minst den matten du *faktisk* trenger på plassen.
    
    **Hva venter deg?**
    * [cite_start]**📚 Infokanal:** Alt om de 10 ulike fagretningene[cite: 9, 29].
    * [cite_start]**📐 Praktisk matte:** Lær om areal, volum og 3-4-5 regelen[cite: 31, 41].
    * [cite_start]**🎮 Quiz:** Test deg selv og samle poeng[cite: 42].
    * [cite_start]**👷 Verksmesteren:** Spør vår AI om alt du lurer på[cite: 5, 7].
    """)
    
    st.divider()
    
    name = st.text_input("Skriv inn navnet ditt for å begynne:", placeholder="Navn...")
    ...
    st.stop() # Stopper her slik at resten av koden ikke kjøres før start

# [cite_start]--- TOPP-RAD (Vises etter start) --- [cite: 4, 5]
...
# [cite_start]--- DATABASE FOR ALLE 10 TEMAER --- [cite: 9 - 29]
data_db = {
    "Anleggsgartner": {
        "beskrivelse": "🌱 Bygger og vedlikeholder uterom, parker og hager. 🧱 Kombinerer levende planter med stein, betong og tre.",
        "verktoy": "Vater, murersnor, steinkutter, gravemaskin.",
        "utdanning": "📜 Vg1 Bygg -> Vg2 Anleggsgartner -> 2 år lærlingtid.",
        "videre": "🎓 Fagskole, mesterbrev eller landskapsarkitektur.",
        "motivasjon": "✨ Liker du å se resultater som vokser? Her setter du spor folk vil nyte i generasjoner!",
        "quiz": ("Hva brukes en murersnor til?", ["Lage rette linjer", "Måle fukt"], "Lage rette linjer")
    },
    "Anleggsteknikk": {
        "beskrivelse": "🚜 Betjener store maskiner for veibygging, tunneler og utgraving. 🏗️ Legger grunnlaget for samfunnet.",
        "verktoy": "Gravemaskiner, hjullastere, dumper, GPS-måleutstyr.",
    ...
    "Overflateteknikk": {
        ...
        "motivasjon": "🌈 Er du kreativ? Her setter du den siste finishen som kunden faktisk ser hver dag!",
    ...
    "Tømrer": {
        "beskrivelse": "🏠 Oppføring av trebygninger fra reisverk til ferdig house. 🔨 Den største faggruppen i bygg.",
...
with tab_matte:
    ...
    if m_kat == "Omkrets":
        [cite_start]st.write("### 📏 Omkrets – Hvor langt er det rundt?") [cite: 31]
        [cite_start]st.write("Omkretsen er summen av alle sidene. [cite: 32]")
        ...
        [cite_start]st.write("### ⬛ Areal – Hvor stor er en flate?") [cite: 33]
        [cite_start]st.write("Areal forteller hvor stor en flate er ($m^2$). [cite: 33]")
        st.latex(r"Areal = L \times B")
        [cite_start]st.write("**Oppgave:** Du skal legge gulv i en bod på 2,5m x 4m. Hvor mange m²?") [cite: 34]
        ans2 = st.radio("Svar:", ["6,5 m²", "10 m²", "8 m²"], index=None, key="m2")
        if st.button("Sjekk Areal"):
            if ans2 == "10 m²":
                [cite_start]st.success("Helt rett! 2,5 * 4 = 10 m²"); st.session_state.points += 5 [cite: 35]

    elif m_kat == "Volum":
        [cite_start]st.write("### 🧊 Volum – Innhold") [cite: 35]
        [cite_start]st.write("Volum forteller hvor mye en gjenstand rommer ($m^3$). [cite: 35]")
        st.latex(r"Volum = L \times B \times H")
        st.write("**Oppgave:** En såle er 5m lang, 2m bred og 0,2m høy. Hvor mange m³ betong trenger du?")
        [cite_start]ans_vol = st.radio("Svar:", ["2,0 m³", "1,0 m³", "7,2 m³"], index=None, key="m_vol") [cite: 36]
        if st.button("Sjekk Volum"):
            if ans_vol == "2,0 m³":
                [cite_start]st.success("Riktig! 5 * 2 * 0,2 = 2,0 m³."); st.session_state.points += 10 [cite: 36]

    elif m_kat == "Prosent & Svinn":
        [cite_start]st.write("### 📈 Prosent og Svinn") [cite: 37]
        [cite_start]st.write("Vi legger til svinn (ofte 10%-15%) fordi noe kappes bort. [cite: 37, 38]")
        [cite_start]st.write("**Oppgave:** Du trenger 50 meter kledning. Med 10% svinn, hvor mye bestiller du?") [cite: 38]
        ans3 = st.radio("Svar:", ["55m", "50,1m"], index=None, key="m3")
        if st.button("Sjekk Svinn"):
            if ans3 == "55m":
                [cite_start]st.success("Riktig! 50 + 5 = 55m."); st.session_state.points += 10 [cite: 38]

    elif m_kat == "Målestokk":
        [cite_start]st.write("### 🗺️ Målestokk") [cite: 39]
        [cite_start]st.write("Målestokk 1:50 betyr at virkeligheten er 50 ganger større enn tegningen. [cite: 39]")
        st.write("**Oppgave:** På tegning (1:50) måler du 10cm. Hvor langt er det i virkeligheten?")
        [cite_start]ans4 = st.radio("Svar:", ["5 meter", "50 cm"], index=None, key="m4") [cite: 40]
        if st.button("Sjekk Målestokk"):
            if ans4 == "5 meter":
                [cite_start]st.success("Riktig! 10cm * 50 = 5m."); st.session_state.points += 10 [cite: 40]

    elif m_kat == "Vinkler":
        [cite_start]st.write("### 📐 Vinkler (3-4-5 regelen)") [cite: 41]
        [cite_start]st.write("Hvis sidene er 3 og 4, må diagonalen være 5 for 90 grader. [cite: 41]")
        st.latex(r"a^2 + b^2 = c^2")
        st.write("**Oppgave:** Sidene er 60cm og 80cm. Hva er diagonalen i vinkel?")
        [cite_start]ans5 = st.radio("Svar:", ["100cm", "140cm"], index=None, key="m5") [cite: 42]
        if st.button("Sjekk Vinkel"):
            if ans5 == "100cm":
                [cite_start]st.success("Vinkelen er 90 grader!"); st.session_state.points += 20; st.balloons() [cite: 42]
...
with tab_quiz:
    ...
    if st.button("Sjekk Quiz"):
        if res == svar:
            [cite_start]st.success("Riktig!"); st.session_state.points += 20; st.balloons(); st.rerun() [cite: 42]
        else:
            [cite_start]st.error("Feil svar, prøv igjen!") [cite: 43]
